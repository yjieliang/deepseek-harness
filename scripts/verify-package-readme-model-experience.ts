...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-kb': { kind: 'indirect', reason: 'The panel reads and writes files through the kb Remote namespace; the model-facing kb_* tools live in @deepseek-ai/dsh-tool-kb and own every model-visible effect.' },
  'packages/client/ui-memory': { kind: 'indirect', reason: 'The browser settings panel edits the user-habits namespace through the wire; the model-facing memory_* tools in dsh-tool-memory and the resident prompt section in dsh-user-habits own every model-visible effect.' },
  'packages/habits/user-habits': { kind: 'indirect', reason: 'The habit contract supplies the resident-section renderer, guard, and consolidation; dsh-habits-settings registers the prompt section the model sees, and dsh-tool-memory/dsh-command-memory own the model-visible tools and command.' },
  'packages/habits/command-memory': { kind: 'indirect', reason: 'The /memory command dispatches without a model turn and registers no prompt, schema, tool, or message; its execution is recorded to the session log only.' },
  'packages/host/kb': { kind: 'indirect', reason: 'The host engine and Remote namespace are the panel\'s data plane; the model-facing kb_* tools live in @deepseek-ai/dsh-tool-kb and own every model-visible effect.' },
  'packages/client/ui-lookup': { kind: 'indirect', reason: 'The browser word card consumes the lookupLlm/explain Remote; dsh-host-lookup-llm owns the model-visible request.' },
