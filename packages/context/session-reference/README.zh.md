...
- `@deepseek-ai/dsh-session-reference/grammar` 子路径导出浏览器安全的编码半边（`SESSION_REFERENCE_SCHEME`、`encodeSessionReferenceUri`、`formatSessionReferenceMention`），不依赖任何 Node 内建模块，因此 Web 客户端共享唯一的规范编码权威，而不是重新实现 base64url。
