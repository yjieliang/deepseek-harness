...
- The `@deepseek-ai/dsh-session-reference/grammar` subpath exports the browser-safe encoding half (`SESSION_REFERENCE_SCHEME`, `encodeSessionReferenceUri`, `formatSessionReferenceMention`) with no Node builtins, so the Web client shares the one canonical encoding authority instead of reimplementing base64url.
