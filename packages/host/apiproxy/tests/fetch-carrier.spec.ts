    ...
    host: {
      async describe(request) {
        return {
          ...
          result: {
            ok: true,
            value: { version: 'v', cwd: '/w', attachedSessions: 0, home: '/h', canOpenPath: true, dshHome: '/w/.dsh', dshHomeSource: 'default' },
          },
        }
      },
      ...
      async readFile(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { path: request.payload.path, content: 'x', truncated: false, lang: null } } }
      },
    },
    workspace: {
      async list(request) {
