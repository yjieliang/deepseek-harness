...
describe('settings domain', () => {
  ...
  it('serves every registered namespace, including one this repository never named', async () => {
    ...
    ctx.settings.register(settingsNamespace('user-habits'), z.object({
      entries: z.array(z.object({ topic: z.string(), value: z.string() })),
    }))
    const api = createApiProxy(ctx, DEFAULTS)
    ...
    expect(value.namespaces.map(view => view.ns)).toEqual([
      ...
      'ui-conversation', 'shell', 'agent-loop', 'web-search-deepseek', 'user-habits',
    ])
    ...
    const habits = expectOk(await api.settings.replace(request({
      ns: 'user-habits',
      section: { entries: [{ topic: 'style', value: '简洁' }] },
    })))
    expect(habits.value).toEqual({ entries: [{ topic: 'style', value: '简洁' }] })
