...
DeepSeek Harness habits 接缝的 settings 后端提供方。把 `global` 层习惯条目存入用户设置命名空间 `user-habits`(schema 校验、串行写入、`settings/updated` 事件流),并注册带预算的常驻提示段——已确认的习惯在删除前进入每次模型请求。

契约、守卫、合并与提交事件在 [`@deepseek-ai/dsh-user-habits`](../user-habits/README.md);本包只实现存储钩子与常驻段。
...
命名空间以 `base: { entries: [] }` 注册,用户在存储任何内容前 section 即可解析。注册是本插件 fiber 上的效果:卸载提供方会一并移除命名空间与提示段。
...
插件挂载且存在条目时,渲染后的 section 在每次装配中进入系统提示:
...
受 `residentTokenBudget` 约束(共享启发式下默认 600 tokens);无条目时提示中不出现该 section。

### KV Cache 效应

section 的名称、顺序与条目排序稳定,条目文本只在习惯提交时变化——习惯写入前,提示前缀在请求间保持可复用。
