...
DeepSeek Harness 的用户习惯记忆接缝。本包是**服务定义(Service Definition)**:抽象 `HabitService` 契约、写入安全守卫、确定性主题合并、品牌化 `HabitId`、类型化 `HabitError`、`user-habits/committed` 事件,以及规范化的常驻段渲染器。提供方实现三个存储钩子;工具、命令与提示段只依赖契约。
...
提供方继承 `HabitService` 并实现 `listAll`、`persistWrite`、`persistRemove`。基类在提供方持久化成功后才发出提交事件,监听方读到的始终是权威存储状态。
...
守卫按来源分层:归一化、隐形字符剥离、非空与预算永远执行;注入与密钥模式对 `agent-proposed` 内容硬拒,`user` 来源的内容仅在 `userConfirmed: true`(人类看过警告并确认)时放行。`guardHabitContent(raw)` 预检候选(不含预算)——提议方在询问人类前使用。

值先做 NFKC 归一化与去首尾空白再校验。预算按 Unicode 码点计,而非 UTF-16 码元。

## Model Experience

无——本包不注册任何面向模型的提示、schema、工具或消息。模型面由消费方拥有。
