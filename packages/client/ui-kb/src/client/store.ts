/**
 * Shared panel open-state controller for the knowledge-base surface. One
...
import type { KbUiState } from './contract.ts'

/** Initial UI state: panel closed. */
const INITIAL: KbUiState = { open: false }

/** Owns the open/close flag the trigger and the panel share. */
export class KbUiController {
  /** Panel open-state snapshot the renderer binds as useKbUi. */
  readonly store: SnapshotStore<KbUiState> = createSnapshotStore(INITIAL)
