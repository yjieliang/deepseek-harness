...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口、输出上限与图片输入开关收在该行的展开区内，右侧是两个无文字的操作——展开与删除。开关会把模型的 `input` 模态列表显式写入（开启为 `['text', 'image']`，关闭为 `['text']`），因此关掉它永远意味着「该模型不接受图片」，而不是悄悄继承一条允许图片的路由；未声明任何模态的行显示为关，因为卡片看不到它继承的路由默认值。开关旁带有一条提示：仅为实际接受图片的模型启用，因为纯文本模型会在网关处拒绝图片请求。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。
