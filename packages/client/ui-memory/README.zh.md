...
Web 记忆界面插件：其浏览器半部分在根作用域的 `settings.section` slot 中注册 `memory` 条目 —— 即列出、添加、编辑和删除全局用户习惯的设置页。它的 Node 半部分刻意留空：页面编辑的 `user-habits` 设置命名空间属于宿主契约，由 `dsh-habits-settings` 拥有，本插件只通过 wire 呈现它（`settings.describe` 读取，`settings.replace` 加 `expectedRevision` 写入）。
