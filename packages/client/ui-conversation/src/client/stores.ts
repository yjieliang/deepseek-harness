...
type ChatActions = {
  ...
  setPreviewFile: (draft: ChatStoreState, target: { path: string; lang: string | null } | null) => void
}
...
export function createChatStore(): EngineStoreHandle<ChatStoreState, ChatActions> {
  return defineStore({
    ...
    init: (): ChatStoreState => ({ selection: null, draft: '', view: null, inspect: null, previewFile: null }),
    ...
    actions: {
      ...
      setPreviewFile: (d, target: { path: string; lang: string | null } | null) => { d.previewFile = target },
    },
  })
}
