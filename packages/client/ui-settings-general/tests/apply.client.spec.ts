...
describe('ui-settings-general apply', () => {
  ...
  it('fills all five seats for declarations before or after apply', async () => {
    ...
    // The boot-home row is this plugin's own loopback-only General item.
    expect(before.slots.entries('settings.general.item')).toHaveLength(1)
    expect(before.slots.entries('settings.general.item')[0]!.options).toMatchObject({ id: 'boot-home' })
