...
- **A modality declaration is not verified, and over-claiming outlives the turn** — nothing interrogates an endpoint for what it accepts, so a model declaring `image` its gateway does not serve is refused by the provider mid-turn rather than here. Prompt admission commits the user message durably before the request is built, so the rejected image stays in the session log and that model keeps re-sending it. Recovery is another image-capable model, a switch to a text-only model (the image degrades to placeholder text on its wire), a fork before the image, or a new session; rolling an unconsumed image message back out of the log on a failed send is deferred.
