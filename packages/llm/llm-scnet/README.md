...
SCNet (National Supercomputing Internet) chat-completions adapter for the harness LLM seam: direct `fetch` + SSE (framed by `eventsource-parser`) translating the platform's OpenAI-compatible wire format into the `StreamChunk` protocol. The hosted model is `DeepSeek-V4-Flash-0731`, so the adapter follows the DeepSeek thinking-mode and tool-call conventions that the SCNet endpoint exposes.

This package owns the `scnet` provider route. Registering another adapter for `scnet` throws `LlmError('DUPLICATE_ADAPTER')`.

The package root exposes the Cordis plugin contract and `ScnetAdapter`; wire serialization, SSE parsing, and chunk translation helpers are not part of that root contract.

## Config

```yaml
- id: llm-scnet
  name: '@deepseek-ai/dsh-llm-scnet'
  config:
    apiKeyEnv: SCNET_API_KEY  # default; resolved per request via ctx.credentials, then the environment
    baseURL: https://api.scnet.cn/api/llm/v1 # optional; $SCNET_BASE_URL then the public API when omitted
...
The plugin registers the single provider route `scnet` together with its resolved `retryPolicy`. A request selects it with `provider: scnet`; its `model` is passed through as the wire `model` string, so using other SCNet-hosted models does not require lifecycle-time registration. Omitting `models` advertises `DeepSeek-V4-Flash-0731` with a 1,000,000-token context window; an explicit list replaces that default, while `models: []` advertises none. Vision models are not advertised by default until their endpoint rollout is complete, but a deployment can add one with `inputModalities: [text, image]`. Catalog entries are exposed through `ctx.llm.listModels('scnet')` for clients such as ACP editors and the Web selector, but remain advisory: unlisted model ids still pass through unchanged. An omitted entry name defaults to its id, and omitted `inputModalities` means `text` only.
...
The one registration-captured fact is the retry policy: when its resolved value changes, the plugin re-registers the route in place (same adapter instance, one synchronous section), so `ctx.llm.providerRetryPolicy('scnet')` always reports the current policy.

The plugin also declares its route in the configurable-provider directory (`ctx.llm.listConfigurableProviders()`): provider `scnet`, settings namespace `llm-scnet`, empty settings path — the whole section is the profile. Configuration surfaces use that entry to offer this adapter alongside dormant pi-ai providers.

## App attribution

Every request carries the shared attribution header from dsh-llm's `attributionHeaders()` - the mandatory `User-Agent` baseline identifying the harness (see [dsh-llm § App attribution](../llm/README.md#app-attribution-attributionts)). SCNet requests get no provider-specific app-attribution headers under this adapter contract. A request whose `GenerateOptions.purpose` is `compaction` (dsh-compaction-basic's auxiliary summarization call) additionally carries `x-deepseek-harness-compact: 1`, so the host can separate compaction traffic from conversation requests.

SCNet request identity is separate from app attribution. After credential resolution, every provider request carries `x-deepseek-harness-user-id` with the stable anonymous id from [`@deepseek-ai/dsh-anonymous-user-id`](../../identity/anonymous-user-id/README.md); a request carrying `GenerateOptions.sessionId` also sends that exact value as `x-deepseek-harness-session-id`, while a direct call without a session omits the session header. Both headers go to the resolved `baseURL` and remain outside the request body and model-visible content.
...
The selected SCNet model receives the harness system prompt, message history, tool schemas, stop sequences, and call config without adapter-authored prompt prose. The vision model also receives retained user and tool-result images as base64 data URLs; an over-budget older image is represented by the documented placeholder. On a prior assistant turn with tool calls, its reasoning content is passed back as required; reasoning from tool-call-free turns is omitted.

#### Token effect

Provider tokenization governs exact text and image-token input. Conditional reasoning passback increases tool-round-trip context, while dropping other reasoning avoids paying those tokens again; cache-read usage is reported when available.

#### KV Cache effect

An unchanged assembled prefix, including deterministically encoded retained images and placeholders, is eligible for cache reuse, which this adapter reports in usage. A model-route change or any upstream prompt, schema, prefix, history, or image-budget change may prevent reuse from the first changed token; reasoning passback appends during tool round trips.
