...
The plugin registers the single provider route `scnet` together with its resolved `retryPolicy`. A request selects it with `provider: scnet`; its `model` is passed through as the wire `model` string, so using other SCNet-hosted models does not require lifecycle-time registration. Omitting `models` advertises `DeepSeek-V4-Flash-0731` with a 1,000,000-token context window; an explicit list replaces that default, while `models: []` advertises none. Catalog entries are exposed through `ctx.llm.listModels('scnet')` for clients such as ACP editors and the Web selector, but remain advisory: unlisted model ids still pass through unchanged. An omitted entry name defaults to its id.
...
The selected `DeepSeek-V4-Flash-0731` model receives the harness system prompt, message history, tool schemas, stop sequences, and call config without adapter-authored prompt prose. On a prior assistant turn with tool calls, its reasoning content is passed back as required; reasoning from tool-call-free turns is omitted.
...
An unchanged assembled prefix is eligible for cache reuse, which this adapter reports in usage. A model-route change or any upstream prompt, schema, prefix, or history change may prevent reuse from the first changed token; reasoning passback appends during tool round trips.
...
- **Serialization flattens user and tool-result content to text blocks** — plugin-added block types are skipped, and empty tool output crosses the wire as the literal `(no output)`. This wire route is text-only: core image blocks degrade to the shared placeholder text (`IMAGE_OMITTED_PLACEHOLDER` from dsh-llm) instead of failing the request, so an image-bearing session can switch to this route.
