...
import type { Config } from '@deepseek-ai/dsh-llm-scnet'
import { assemble, type AssembledResult } from './assemble.ts'

/**
 * Real-API e2e for the direct-fetch adapter: DeepSeek-V4-Flash-0731 hosted on
 * SCNet across thinking modes and both official effort levels. Key-gated —
 * skips entirely without $SCNET_API_KEY (see vitest.e2e.config.ts).
 */

const FLASH = 'DeepSeek-V4-Flash-0731'
...
describe.skipIf(!process.env.SCNET_API_KEY)('llm-scnet e2e (real API)', () => {
  it('serves a real request with the key held only by a credentials-local document', async () => {
    const key = process.env.SCNET_API_KEY
    if (key === undefined) throw new Error('e2e ran without SCNET_API_KEY')
    ...
    try {
      ...
      await writeFile(join(dir, '.credentials.yaml'), `SCNET_API_KEY: ${JSON.stringify(key)}\n`, { mode: 0o600 })
  ...
  it.each(['high', 'max'] as const)(
    'flash + thinking enabled (effort %s): tool-call round trip with reasoning passback',
    async (effort) => {
  ...
  it('flash + thinking disabled: plain generation without reasoning blocks', async () => {
