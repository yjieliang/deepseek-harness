...
import * as LlmScnet from '@deepseek-ai/dsh-llm-scnet'
import { ScnetAdapter, resolveAdapterOptions } from '@deepseek-ai/dsh-llm-scnet'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'dsh-llm-scnet-'))
...
async function harness(baseURL: string, config: object = {}) {
  ...
  vi.stubEnv('SCNET_API_KEY', 'test-key')
  const ctx = new Context()
  await ctx.plugin(LlmRuntime)
  await ctx.plugin(LlmScnet, { baseURL, ...config })
  return ctx
}

/** Direct adapter over the plugin's real resolve step, with a static key. */
function adapterOf(config: Partial<LlmScnet.Config> & { apiKey?: string } = {}): ScnetAdapter {
  const { apiKey, ...rest } = config
  return new ScnetAdapter({
...
describe('ScnetAdapter against a mock server', () => {
  it('streams a text generation end to end through the assembler', async () => {
    ...
    // App attribution and SCNet request identity are independent wire facts.
  ...
  it('uses the configured maxTokens default and preserves an explicit request cap', async () => {
    ...
    await assemble(ctx, { model: 'DeepSeek-V4-Flash-0731', messages: [] })
    await assemble(ctx, { model: 'DeepSeek-V4-Flash-0731', messages: [], maxTokens: 8_192 })
  ...
  it('publishes only off and omits the wire effort when thinking is disabled', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('scnet', 'DeepSeek-V4-Flash-0731'))
  ...
  it.each([
    ...
    const ctx = await harness(server.url)
    const result = await assemble(ctx,{ model: 'DeepSeek-V4-Flash-0731', messages: [] })
  ...
  it('parses a future Retry-After HTTP date and the SCNet request-id fallback', async () => {
  ...
  it('reports a transport failure with the endpoint in the message', async () => {
    ...
    expect(result.finish).toMatchObject({
      ...
      failure: {
        code: 'TRANSPORT',
        message: 'SCNet API request to http://127.0.0.1:1 failed',
      },
    })
  })
  ...
  it('throws EMPTY_RESPONSE when the response has no body', async () => {
    ...
    try {
      const iterate = async (): Promise<void> => {
        for await (const _chunk of adapter.stream({ provider: 'scnet', model: 'm', messages: [] })) { /* drain */ }
      }
  ...
  it('classifies an abrupt body close as TRANSPORT', async () => {
    ...
    expect(result.finish.failure.message).toMatch(/^SCNet API stream from .* failed$/)
  })
  ...
  it('renders a non-Error transport rejection without losing its cause', async () => {
    ...
    try {
      ...
      await expect(drain()).rejects.toMatchObject({
        message: 'SCNet API request to https://example.invalid failed',
        code: 'TRANSPORT',
        cause: 'offline',
      })
    } finally {
  ...
  it('keeps an idle provider read alive through SSE comments', async () => {
    ...
    try {
      const chunks: string[] = []
      const drain = (async () => {
        for await (const chunk of adapter.stream({ provider: 'scnet', model: 'm', messages: [] })) {
...
describe('plugin registration and config', () => {
  it('keeps wire helpers off the package root', () => {
    for (const helper of [
      ...
      'mapUsage',
      'translate',
    ]) expect(LlmScnet).not.toHaveProperty(helper)
  })
  ...
  it('registers the deepseek provider and unregisters on dispose (HMR safety)', async () => {
    ...
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const fiber = await ctx.plugin(LlmScnet, {
      baseURL: server.url,
    })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'scnet', name: 'SCNet' }])
    expect(ctx.llm.listConfigurableProviders()).toEqual([{
      provider: 'scnet',
      displayName: 'SCNet',
      settingsNs: 'llm-scnet',
      settingsPath: [],
    }])
    await fiber.dispose()
  ...
  it('registers retryPolicy from the provider config', async () => {
    ...
    expect(ctx.llm.providerRetryPolicy('scnet')).toEqual({
      mode: 'always',
      initialDelayMs: 25,
      maxDelayMs: 100,
      jitterRatio: 0.2,
  ...
  it('owns the scnet provider and advertises the default models', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmScnet, { baseURL: 'http://127.0.0.1:1' })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'scnet', name: 'SCNet' }])
    await expect(ctx.llm.listModels('scnet')).resolves.toEqual([
      { provider: 'scnet', id: 'DeepSeek-V4-Flash-0731', name: 'DeepSeek-V4-Flash-0731', inputModalities: ['text'] },
    ])
    await expect(ctx.llm.resolveModelInfo('scnet', 'DeepSeek-V4-Flash-0731'))
      .resolves.toMatchObject({
        provider: 'scnet',
  ...
  it.each(['off', 'max'] as const)('uses the configured %s reasoning default', async (effort) => {
    ...
    await expect(ctx.llm.resolveModelInfo('scnet', 'unlisted-pass-through'))
      .resolves.toMatchObject({
        reasoning: {
          efforts: [
            { id: ReasoningEffortId('off'), name: 'Off' },
  ...
  it.each(['high', 'max'] as const)(
    ...
    async (reasoningEffort) => {
      const ctx = new Context()
      await ctx.plugin(LlmRuntime)
      await expect(ctx.plugin(LlmScnet, {
        baseURL: 'http://127.0.0.1:1',
        thinking: 'disabled',
        reasoningEffort,
  ...
  it('accepts disabled thinking with off at the resolver boundary', async () => {
    ...
    await expect(adapter.resolveModel('scnet', 'pass-through')).resolves.toMatchObject({
      reasoning: {
        efforts: [{ id: ReasoningEffortId('off'), name: 'Off' }],
        defaultEffort: ReasoningEffortId('off'),
      },
  ...
  it('uses the default model catalog when apply is called directly', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    LlmScnet.apply(ctx, { baseURL: 'http://127.0.0.1:1' })
    await expect(ctx.llm.listModels('scnet')).resolves.toEqual([
      { provider: 'scnet', id: 'DeepSeek-V4-Flash-0731', name: 'DeepSeek-V4-Flash-0731', inputModalities: ['text'] },
    ])
  })
  ...
  it('advertises configured models without restricting arbitrary request ids', async () => {
    ...
    await expect(ctx.llm.listModels('scnet')).resolves.toEqual([
      { provider: 'scnet', id: 'private-fast', name: 'private-fast', inputModalities: ['text'] },
      { provider: 'scnet', id: 'private-reasoner', name: 'Private Reasoner', description: 'Higher reasoning budget', inputModalities: ['text'] },
    ])
    await expect(ctx.llm.resolveModelInfo('scnet', 'private-fast'))
      .resolves.toMatchObject({ context: { contextWindow: 32_000 } })
    await expect(ctx.llm.resolveModelInfo('scnet', 'private-reasoner'))
      .resolves.toMatchObject({
        name: 'Private Reasoner',
        description: 'Higher reasoning budget',
      })
    await expect(ctx.llm.resolveModelInfo('scnet', 'arbitrary-unlisted'))
      .resolves.toMatchObject({
        context: { contextWindow: 1_000_000 },
        defaultMaxTokens: 256_000,
      })
  ...
  it('uses exact model capacity before the adapter-wide default', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('scnet', 'inherits-default'))
      .resolves.toMatchObject({ context: { contextWindow: 256_000 } })
    await expect(ctx.llm.resolveModelInfo('scnet', 'exact-override'))
  ...
  it('prefers a model\'s own output cap over the profile default', async () => {
    ...
    await expect(adapter.resolveModel('scnet', 'capped'))
      .resolves.toMatchObject({ defaultMaxTokens: 512 })
    await expect(adapter.resolveModel('scnet', 'uncapped'))
      .resolves.toMatchObject({ defaultMaxTokens: 4096 })
    await expect(adapter.resolveModel('scnet', 'not-in-catalog'))
      .resolves.toMatchObject({ defaultMaxTokens: 4096 })
  })
  ...
  it('falls back to SCNET_API_KEY and SCNET_BASE_URL env vars', async () => {
    vi.stubEnv('SCNET_API_KEY', 'env-key')
    vi.stubEnv('SCNET_BASE_URL', 'http://127.0.0.1:1')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmScnet, {})
    expect(ctx.llm.listProviders()).toEqual([{ id: 'scnet', name: 'SCNet' }])
  ...
  it('loads keyless, keeps the catalog browsable, and fails the request actionably', async () => {
    ...
    expect(ctx.llm.listProviders()).toEqual([{ id: 'scnet', name: 'SCNet' }])
    await expect(ctx.llm.listModels('scnet')).resolves.toHaveLength(1)
    const first = await assemble(ctx, { model: 'DeepSeek-V4-Flash-0731', messages: [] })
    expect(first.finish).toMatchObject({ kind: 'error', failure: { code: 'MISSING_CREDENTIAL' } })
    // The guidance leads with the managed credential store.
    const second = await assemble(ctx, { model: 'DeepSeek-V4-Flash-0731', messages: [] })
    ...
    expect(second.finish.failure.message)
      .toMatch(/store SCNET_API_KEY through the credentials service.*export SCNET_API_KEY/s)
  })
  ...
  it('reads the ambient variable when no credentials seam is mounted', async () => {
    ...
    vi.stubEnv('SCNET_API_KEY', 'ambient-key')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmScnet, { baseURL: server.url })
    await assemble(ctx, { model: 'DeepSeek-V4-Flash-0731', messages: [] })
  ...
  it('prefers explicit config over env for key and base URL', async () => {
    vi.stubEnv('SCNET_API_KEY', 'env-key')
    vi.stubEnv('SCNET_BASE_URL', 'http://env-host:1')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
  ...
  it('uses SCNET_BASE_URL when config omits baseURL', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    vi.stubEnv('SCNET_BASE_URL', server.url)
    vi.stubEnv('SCNET_API_KEY', 'test-key')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmScnet, {})
  ...
  it('takes SCNET_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.dsh/.env', values: { SCNET_BASE_URL: 'https://user.example' } },
    ])
    ...
    const project = createLaunchEnvironmentSnapshot([
      { source: 'project-env', path: '/work/.env', values: { SCNET_BASE_URL: 'https://project.example' } },
    ])
    ...
    const shell = createLaunchEnvironmentSnapshot([
      { source: 'process', values: { SCNET_BASE_URL: 'https://stale.example' } },
    ])
  ...
  it('defaults to the public base URL without config or env', async () => {
    vi.stubEnv('SCNET_API_KEY', 'k')
    vi.stubEnv('SCNET_BASE_URL', undefined)
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
  ...
  it('adapter is constructible directly for embedding over the shared resolver', async () => {
    ...
    expect(adapter).toBeInstanceOf(ScnetAdapter)
    ...
    await expect(adapter.listModels('scnet')).resolves.toHaveLength(1)
  })
