 ...
 * settings-file, credentials-local, and llm-scnet boot from a test-only
...
async function loadComposition(
  ...
  if (options.withDynamic && fresh) {
    ...
    await writeFile(credentialsPath, 'SCNET_API_KEY: boot-key\n', { mode: 0o600 })
  }
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@deepseek-ai/dsh-llm-scnet', LlmScnet],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-scnet real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    await writeFile(settingsPath, `llm-scnet:\n  baseURL: ${serverB.url}\n`)
    await vi.waitFor(() => {
      expect((ctx.get('settings')!.get(NS) as { baseURL?: string }).baseURL).toBe(serverB.url)
    }, { timeout: 5000 })
    await writeFile(credentialsPath, 'SCNET_API_KEY: rotated-key\n', { mode: 0o600 })
  ...
  it('keeps a stored key writable and rotatable across a real restart', async () => {
    // No ambient SCNET_API_KEY: the shipped surfaces do not hoist
    ...
    await assemble(boot.ctx, { model: 'DeepSeek-V4-Flash-0731', messages: [] })
    ...
    await assemble(restarted.ctx, { model: 'DeepSeek-V4-Flash-0731', messages: [] })
  ...
  it('boots the same adapter on entry config alone, resolving the reference from the environment', async () => {
    ...
    vi.stubEnv('SCNET_API_KEY', 'entry-key')
