/**
 * Register a {@link ScnetAdapter} for the `scnet` provider route on
 * `ctx.llm`, with connection facts resolved per request instead of frozen at
 * load: the plugin layers its `cordis.yml` entry config under the optional
 * `llm-scnet` user-settings section (`ctx.settings`) and resolves the API
 * key through the optional credential seam (`ctx.credentials`), so a changed
 * base URL, catalog, or key reaches the very next request without restarting
 * anything, while an in-flight stream keeps the facts it started with. The
 * one registration-captured fact — the retry policy — re-registers the route
 * in place when it changes.
 * @module @deepseek-ai/dsh-llm-scnet
 */
...
import type { ScnetCatalogModel, ScnetConnectionOptions } from './adapter.ts'
...
export type { ScnetAdapterOptions, ScnetCatalogModel, ScnetConnectionOptions } from './adapter.ts'
export type { RequestDefaults } from './serialize.ts'
export type * from './types.ts'

export const name = 'llm-scnet'
export const inject = ['llm']

const NS = settingsNamespace('llm-scnet')
const DEFAULT_API_KEY_ENV = 'SCNET_API_KEY'
/** The single provider route this plugin owns. */
const PROVIDER = 'scnet'
...
const catalogModel: z<ScnetCatalogModel> = z.object({
...
/** Public API default; the internal endpoint comes from $SCNET_BASE_URL. */
export const PUBLIC_BASE_URL = 'https://api.scnet.cn/api/llm/v1'

/** Environment variable naming this provider's endpoint, honored only from trusted layers. */
const BASE_URL_ENV = 'SCNET_BASE_URL'
 ...
 * newer key.
 */
export type ResolvedScnetOptions = ScnetConnectionOptions

/** Resolve, validate, and detach the advisory model catalog. */
function resolveModels(models: readonly ScnetCatalogModel[] | undefined): ScnetCatalogModel[] {
  const seen = new Set<string>()
  return (models ?? DEFAULT_MODELS).map((model) => {
    if (model.id.length === 0) throw new Error('llm-scnet: catalog model ids must be non-empty')
    if (model.name !== undefined && model.name.length === 0) {
      throw new Error(`llm-scnet: catalog model "${model.id}" has an empty name`)
    }
    if (model.contextWindow !== undefined
      && (!Number.isInteger(model.contextWindow) || model.contextWindow <= 0)) {
      throw new Error(
        `llm-scnet: catalog model "${model.id}" contextWindow must be a positive integer`,
      )
    }
 ...
 */
export function resolveAdapterOptions(config: Config, environment?: LaunchEnvironmentSnapshot): ResolvedScnetOptions {
  if (config.thinking === 'disabled'
    && config.reasoningEffort !== undefined
    && config.reasoningEffort !== 'off') {
    throw new Error('llm-scnet: only reasoningEffort "off" can be configured when thinking is disabled')
  }
  if (config.defaultContextWindow !== undefined
    && (!Number.isInteger(config.defaultContextWindow) || config.defaultContextWindow <= 0)) {
    throw new Error('llm-scnet: defaultContextWindow must be a positive integer')
  }
  if (config.maxTokens !== undefined
    && (!Number.isSafeInteger(config.maxTokens) || config.maxTokens <= 0)) {
    throw new Error('llm-scnet: maxTokens must be a positive safe integer')
  }
...
export function apply(ctx: Context, config: Config): void {
  ...
  let lastGood: ResolvedScnetOptions | undefined
  const options = (): ResolvedScnetOptions => {
    const raw = current()
    ...
    try {
      ...
      lastRaw = raw
      ctx.logger.error('llm-scnet: keeping the last good configuration after an invalid settings section')
  ...
  const resolveApiKey = async (connection: ResolvedScnetOptions): Promise<string> => {
    ...
    if (credentials !== undefined) {
      ...
      if (hit !== undefined) return assertUsableApiKey(hit.value, 'llm-scnet', ref)
    } else {
      // Without the seam there is no managed store to rank against, so the
      // environment is the whole credential plane.
      const ambient = launchEnvironmentOf(ctx).get(ref)
      if (ambient !== undefined && ambient.value.length > 0) {
        return assertUsableApiKey(ambient.value, 'llm-scnet', ref)
      }
    }
    throw new LlmError(
      `llm-scnet: no API key for provider route "${PROVIDER}"; store ${ref} through the credentials`
