/**
 * `ScnetAdapter`: fetch + SSE against an SCNet (OpenAI-compatible)
 * chat-completions endpoint, emitting harness StreamChunks. The adapter is
 * transport-only: connection facts arrive through a thunk resolved once per
 * operation and the bearer token through a per-request resolver, so the
 * registering plugin owns validation, layering, and credential policy.
 *
 * @module dsh-llm-scnet/adapter
 */
 ...
 */
export interface ScnetConnectionOptions {
  ...
  models: readonly ScnetCatalogModel[]
...
/** Constructor options for {@link ScnetAdapter}: the operation-local resolution hooks the plugin owns. */
export interface ScnetAdapterOptions {
  /** Current validated connection facts; called once per operation. */
  options: () => ScnetConnectionOptions
  /**
   ...
   */
  resolveApiKey: (connection: ScnetConnectionOptions) => Promise<string>
...
function modelInfo(provider: string, model: ScnetCatalogModel): LlmModelInfo {
  return {
    provider,
    id: model.id,
 ...
 */
export class ScnetAdapter extends LlmAdapter {
  constructor(private readonly config: ScnetAdapterOptions) {
    super()
  }

  override providerInfo(provider: string): LlmProviderInfo {
    return { id: provider, name: 'SCNet' }
  }
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (hasImages) {
      ...
      if (model?.inputModalities?.includes('image') !== true) {
        throw new LlmError(
          `SCNet model "${options.model}" does not accept image input.`,
          'UNSUPPORTED_CONTENT',
        )
      }
      ...
      if (attachments === undefined) {
        throw new LlmError(
          'SCNet image conversion requires the durable attachment service.',
          'UNSUPPORTED_CONTENT',
        )
      }
    }
    ...
    try {
      ...
      if (timeoutOf(watchdog.signal, STREAM_IDLE_TIMEOUT_CODE) !== undefined) {
        throw new LlmError(
          `SCNet stream idle timeout after ${connection.streamIdleTimeoutMs}ms`,
          'TIMEOUT',
          { cause: error },
        )
      }
      if (options.signal?.aborted) {
        throw new LlmError('SCNet request aborted by caller', 'ABORTED', { cause: error })
      }
      if (error instanceof LlmError) throw error
      throw new LlmError(`SCNet API stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    } finally {
      consumer.abort('SCNet stream consumer stopped')
  ...
  private async * request(
    ...
    signal: AbortSignal,
    connection: ScnetConnectionOptions,
    apiKey: string,
    ...
    try {
      ...
      throw new LlmError(
        `SCNet API request to ${connection.baseURL} failed`,
        'TRANSPORT',
        { cause: error },
      )
    }
    ...
    if (!response.ok) {
      let message = `SCNet API error (HTTP ${response.status})`
    ...
    if (!response.body) {
      throw new LlmError('SCNet API returned no response body', 'EMPTY_RESPONSE')
    }
