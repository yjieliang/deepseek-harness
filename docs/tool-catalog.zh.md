...
来源：[`packages/host/tool-kb/src/index.ts`](../packages/host/tool-kb/src/index.ts)
...
16 个 kb_* 工具是知识库能力在 ctx.kb 之上的 agent 平面消费方；批量操作以 batchConfirmN 门控并先预览后执行，未挂 web 服务时 kb_clip 降级为仅存 URL+标题。
