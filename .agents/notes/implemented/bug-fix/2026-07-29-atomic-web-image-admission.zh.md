...
包含图片的提示词准入与并发模型选择之间具有确定的先后顺序，纯文本目标无法使已获准入但尚未发布的图片搁浅。模型选择可能等待正在进行的图片准入完成，而无关提示词仍按现有方式并发处理。即使图片仍然存在，模型选择也会接受纯文本目标——适配器会把图片降级为占位文本，而该顺序保证响应中的降级标记反映确切的待发布与派生图片状态（见[图片降级笔记](../feature/2026-08-15-text-only-switch-image-degradation.md)）。压缩仍可从派生历史中移除旧图片。
