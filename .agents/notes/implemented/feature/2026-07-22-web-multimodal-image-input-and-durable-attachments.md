...
| `packages/llm/llm-deepseek` | Degrade image content to placeholder text explicitly. |
...
The implemented slice includes the attachment seam, role-neutral image block, Pi-AI input conversion, DeepSeek image degradation, durable host ordering, Web upload/read protocol, current image-limit enforcement, bounded Web request bodies, in-memory draft images, paste/drop rail, user and assistant history rendering, single-click preview, compaction handling, and keyless assembled Web coverage.
...
- Adapter and compaction tests cover native Pi-AI image conversion, late attachment-service composition, text-only placeholder degradation, recursively nested tool-result images, preserved summary input, and explicit image-output rejection.
