...
- **Navigation column**: status smart folders (收集箱 / 已归档 / 已存档) with per-status counts from `kb.stats`, a directory tree (built client-side from the flat `kb.dirs` list) with inline create/rename through the new `kb.createDir` / `kb.renameDir`, a tag index with counts from `kb.tags`, and the trash as a sibling view.
