...
点击工具行中的文件路径会在宿主上打开该路径。非文件工具行只是不可交互的摘要（行内已有的展开开关仍保留）。远程或非回环客户端无法调用 `host.openPath`。Host 或操作系统拒绝由聊天视图拥有：它展示抛出的原因，并对同一路径提供重试（[打开失败](../bug-fix/2026-08-18-tool-row-file-open-failure.md)）。可读的本地文件现在改为页内预览，而不再在系统里打开——即本 note 曾拒绝的应用内预览——系统交接保留为读不了部分的回退（[in-page file preview](2026-08-22-web-in-page-file-preview.md)）；仅回环的权限栅栏对两者均不变。
