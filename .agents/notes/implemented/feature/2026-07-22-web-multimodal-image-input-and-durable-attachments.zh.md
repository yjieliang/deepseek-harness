...
压缩会把选定的会话前缀（包含图片引用）回放到已配置的摘要生成路径中。支持视觉的路径会通过适配器解析这些引用；仅文本路径会把它们降级为共享占位文本，而不是静默丢弃视觉上下文。合成的检查点仍仅包含文本，`compaction-basic` 会以 `UNSUPPORTED_CONTENT` 拒绝包含图片的摘要输出。
...
| `packages/llm/llm-deepseek` | 明确地将图片内容降级为占位文本。 |
...
已实现的范围包括附件 seam、角色无关的图片块、Pi-AI 输入转换、DeepSeek 图片降级、宿主持久化顺序、Web 上传与读取协议、当前图片限制执行、大小受限的 Web 请求体、内存草稿图片、粘贴与拖放附件栏、用户与助手历史图片渲染、单击预览、压缩处理，以及组装后无需密钥的 Web 覆盖。
...
- 适配器与压缩测试覆盖 Pi-AI 原生图片转换、后置附件服务组合、仅文本占位降级、递归嵌套在工具结果中的图片、保留摘要输入，以及明确拒绝图片输出。
